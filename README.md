<p align="center" width="60%">
<img src="LOGO.png"  width="40%" height="40%">
</p>
  
# <div align="center">LLMDataHub: Awesome Datasets for LLM Training </div>
----------------------------------
<p align="center">
  🔥 <a href="DATASETS.md#general_aligment">Alignment Datasets</a> • 💡 <a href="DATASETS.md#domain-specific">Domain-specific Datasets</a> • :atom: <a href="DATASETS.md#pretrain">Pretraining Datasets</a> • 🖼️ <a href="DATASETS.md#multimodal">Multimodal Datasets</a> <br> 
</p>

<p align="center">
<img alt="GitHub last commit" src="https://img.shields.io/github/last-commit/Project-Hierion/LLMDataHub"> <img alt="GitHub Repo stars" src="https://img.shields.io/github/stars/Project-Hierion/LLMDataHub"> <img alt="Repo Automation Status" src="https://github.com/Project-Hierion/LLMDataHub/actions/workflows/vault-check.yml/badge.svg">
</p>

---

## 🌱 A Partnership, Not a Takeover

This repository is a maintained fork of the original [LLMDataHub](https://github.com/Zjh-819/LLMDataHub) by Junhao Zhao.

**We are not taking over. We are not rebranding. We are simply keeping the garden growing.**

- **Original authors:** Junhao Zhao, advised by Prof. Wanyun Cui
- **Maintained by:** Project Hierion
- **License:** MIT (original) + AGPLv3+Commons Clause (automation)
- **Purpose:** To keep this valuable resource alive and accessible

**Credit where credit is due. Gratitude where gratitude belongs.**

---

## What This Is

A curated collection of open-source resources for LLM training, fine-tuning, and evaluation.

**No corporate bullshit. No HF. No Nvidia.**

---

## What We Index

| Section | What You'll Find |
|---|---|
| [DATASETS.md](DATASETS.md) | Curated datasets for pretraining, SFT, RLHF, DPO, and evaluation |
| [MODELS.md](MODELS.md) | Open-weight models, LoRAs, adapters, and checkpoints |
| [PAPERS.md](PAPERS.md) | Research papers with code links and dataset references |
| [TOOLS.md](TOOLS.md) | Frameworks, libraries, evaluation harnesses, and deployment tools |
| [PLATFORMS.md](PLATFORMS.md) | Larger infrastructure pieces and data/AI platforms |
| [AWESOME-LISTS.md](AWESOME-LISTS.md) | Curated lists of resources — a catalog of catalogs |

---

## How to Use This

1. Browse the section you need
2. Click through to the source
3. Use, contribute, and share

---

## How to Contribute

Found something we missed? Open an issue or a pull request.

---

## The Mycelium

This project is part of the [Project Hierion](https://github.com/Project-Hierion) ecosystem. We believe in open infrastructure, not corporate consolidation.

---

## Contact 📬

**Original project authors:**
- [Junhao Zhao](zhaol9555@gmail.com) 📧  
- Advised by [Prof. Wanyun Cui](https://cuiwanyun.github.io/) [![GitHub.io](https://img.shields.io/badge/GitHub.io-@cuiwanyun-green.svg)](https://cuiwanyun.github.io/)

---

**Project Hierion maintainers:**
- [project-hierion@proton.me](mailto:project-hierion@proton.me)
- **ORCID:** [![ORCID](https://img.shields.io/badge/ORCID-0009--0000--8877--2731-A6CE39?style=flat&logo=orcid&logoColor=white)](https://orcid.org/0009-0000-8877-2731)

*Our work is open, traceable, and part of the scientific record.*

---

*With gratitude and much respect to the community, for providing such useful resources, [Project-Hierion](https://github.com/Project-Hierion) will attempt to maintain and keep this forked branch of the original [LLMDataHub](https://github.com/Zjh-819/LLMDataHub) by Junhao Zhao, up to date and current. 🙏*
